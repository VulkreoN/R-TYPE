...
namespace R_TYPE {
    ...
    std::shared_ptr<Entity> GameSystem::createEnnemy(int name, int posX, int posY, float angle, Ennemy::Type type)
    {
        std::shared_ptr<Entity> entity = std::make_shared<Entity>();
        std::shared_ptr<Position> component2 = std::make_shared<Position>(posX, posY);
        std::shared_ptr<Sprite> component = std::make_shared<Sprite>(name, *component2, angle);
    ...
    std::shared_ptr<Entity> GameSystem::createProjectiles(int name, Position pos, Velocity velocity, bool byPlayer)
    {
        std::shared_ptr<Entity> entity = std::make_shared<Entity>();
        ...
        ButtonCallbacks shoot (
           [](SceneManager &scene) {
                ...
                std::shared_ptr<Entity> shoot = GameSystem::createProjectiles
                    (54, Position(pos->getPosition().x + 20, pos->getPosition().y +10), Velocity(0.1f, 0), true);
